...
public class LangGen extends LanguageProvider {
    ...
    protected void addTranslations() {
        ...
        for (Supplier<Glyph> supplier : arsNouveauAPI.getGlyphItemMap().values().stream().filter(tab -> tab.get().spellPart.getRegistryName().getNamespace().equals(ArsCaelum.MODID)).toList()) {
        ...
        for (RitualTablet i : arsNouveauAPI.getRitualItemMap().values().stream().filter(tab -> tab.ritual.getRegistryName().getNamespace().equals(ArsCaelum.MODID)).toList()) {
